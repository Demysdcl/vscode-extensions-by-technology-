    ...
    async getDefinitions(document, position) {
        ...
        const result = await Promise.all(defs.definitions.map(async (def) => {
            ...
            if (utils_3.isNoTextSpanInGeneratedCode(snapshot.getFullText(), def.textSpan)) {
                return vscode_languageserver_1.LocationLink.create(utils_1.pathToUrl(def.fileName), utils_2.convertToLocationRange(fragment, def.textSpan), utils_2.convertToLocationRange(fragment, def.textSpan), utils_2.convertToLocationRange(mainFragment, defs.textSpan));
            }
        }));
