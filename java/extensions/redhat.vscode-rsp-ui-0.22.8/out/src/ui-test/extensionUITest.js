...
function extensionUIAssetsTest() {
    describe('Verify extension\'s base assets are available after installation', () => {
        ...
        it('Servers activity bar is available', function () {
            return __awaiter(this, void 0, void 0, function* () {
                this.timeout(5000);
                const viewControls = yield new vscode_extension_tester_1.ActivityBar().getViewControls();
                chai_1.expect(viewControls.map(item => item.getTitle())).to.include(adaptersContants_1.AdaptersConstants.RSP_CONNECTOR_NAME);
                const serversView = new vscode_extension_tester_1.ActivityBar().getViewControl(adaptersContants_1.AdaptersConstants.RSP_CONNECTOR_NAME);
                chai_1.expect(serversView).not.undefined;
                const serversBar = yield serversView.openView();
                chai_1.expect(serversBar).not.undefined;
                chai_1.expect(yield serversBar.getTitlePart().getTitle()).to.equal(adaptersContants_1.AdaptersConstants.RSP_ATIVITY_BAR_TITLE);
            });
        });
        it('Action button from Servers activity bar is available', function () {
            return __awaiter(this, void 0, void 0, function* () {
                this.timeout(5000);
                const serversView = new vscode_extension_tester_1.ActivityBar().getViewControl(adaptersContants_1.AdaptersConstants.RSP_CONNECTOR_NAME);
                const serversBar = yield serversView.openView();
                const titlePart = serversBar.getTitlePart();
                const actionsButton = yield titlePart.getActions();
                chai_1.expect(actionsButton.length).to.equal(1);
                chai_1.expect(actionsButton[0].getTitle()).to.include('Create New Server');
            });
        });
